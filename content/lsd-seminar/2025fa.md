...
isolation. Given a history {{< math >}} $H$ {{</ math >}} of size {{< math >}} $n$ {{</ math >}} and
{{< math >}} $k$ {{</ math >}} sessions, AWDIT tests whether $H$ satisfies the most common weak
isolation levels of Read Committed (RC), Read Atomic (RA), and Causal Consistency (CC) in time {{<
math >}} $O(n^{3/2})$ {{</ math >}}, {{< math
>}} $O(n^{3/2})$ {{</ math >}}, and {{< math >}} $O(n \cdot k)$ {{</ math >}}, respectively,
improving significantly over the state of the art. Moreover, we prove that AWDIT is essentially
optimal, in the sense that there is a conditional lower bound of {{< math >}} $n^{3/2}$ {{</ math
>}} for any weak isolation level between RC and CC. Our experiments show that AWDIT is significantly
faster than existing, highly optimized testers; e.g., for the ~20% largest histories, AWDIT obtains
an average speedup of {{< math >}} $245\times$ {{</ math >}}, {{< math >}} $193\times$ {{</ math
>}}, and {{< math >}} $62\times$ {{</ math >}} for RC, RA, and CC, respectively, over the best
baseline.
